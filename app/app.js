...
//The initial api call is slow but there is a lack of params available with this api.
	...
	axios
		...
		.then((response) => {
			...
			data.forEach((element) => {
				...
				if (
					element.type != 'Hero' &&
					element.type != 'Hero Power' &&
					!element.cardId.includes('DFX') &&
					element.img
				) {
					cards.push(element)
				}
		...
		.catch(
			(error) =>
				(document.getElementById('cards').innerHTML = 'Unable To find Cards... ' + error)
		)
}
	...
	cards.forEach((element) => {
		if (ctr >= startIndex && ctr < endIndex) {
			...
			document.getElementById('cards').innerHTML +=
				...
				element.img +
				' \'alt="Failed to Load Image" onerror="this.src=\'resources/cardPlaceHolder.png\'";> ' +
				'<div class="card-body">' +
				'<h4 class="card-title"> ' +
				element.name +
				'</h4>' +
				...
				element.playerClass +
				'.</p>' +
				" <button class=\"btn btn-primary\" id='addCard' data-id='" +
				element.cardId +
				"'>" +
				'ADD CARD' +
				'</button>' +
	...
	if (myDeck.length == 0) {
		...
		myDeck.forEach((element) => {
			document.getElementById('cards').innerHTML +=
				...
				element.playerClass +
				'.</p>' +
				" <button class=\"btn btn-primary\" id='removeCard' data-id='" +
				element.cardId +
				"'>" +
				'Remove Card' +
				'</button>' +
...
function addCardToDeck(newCardId) {
	for (let element of cards) {
		if (element.cardId == newCardId) {
			const exists = myDeck.find((card) => card.cardId === element.cardId)
			if (!exists) {
				element.amount = 1
				myDeck.push(element)
...
function removeCardFromDeck(oldCardId) {
	const card = myDeck.find((card) => card.cardId === oldCardId)
	if (card.amount === 2) {
		card.amount = 1
	} else {
